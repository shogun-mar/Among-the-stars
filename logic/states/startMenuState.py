...
def handle_start_menu_events_mouse(game, button, mouse_pos):
    if play_text_rect.collidepoint(mouse_pos) and button == 1: #Left mouse button
        ...
        game.last_hyperspace_travel_time = game.current_time #Make it so the cooldown bar properly starts when the game is started
        game.last_attack_time = game.current_time #Make it so the attack cooldown circle properly starts when the game is started
